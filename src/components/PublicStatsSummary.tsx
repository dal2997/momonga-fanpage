...
export default function PublicStatsSummary({
  ...
  // ✅ 라이트에서 "너무 다크"로 보이지 않게: bg를 더 투명하게 + 텍스트 대비만 확보
  const pill =
    "rounded-full border px-3 py-1 text-sm transition " +
    "border-black/10 bg-black/[0.03] text-zinc-700 hover:bg-black/[0.05] hover:text-zinc-900 " +
    "dark:border-white/10 dark:bg-white/[0.06] dark:text-white/70 dark:hover:bg-white/[0.10] dark:hover:text-white";

  const subtle = "text-zinc-500 dark:text-white/50";
  const heading = "text-zinc-900 dark:text-white";
  const subtext = "text-zinc-600 dark:text-white/60";
          ...
          <div>
            ...
            <div className={`mt-3 flex items-center gap-3 text-xs ${subtle}`}>
              <span className="inline-flex items-center gap-2">
                <span className="h-[2px] w-6 rounded bg-black/45 dark:bg-white/40" />
                수집완료(실선)
              </span>
              <span className="inline-flex items-center gap-2">
                <span
                  className="h-[2px] w-6 rounded bg-black/35 dark:bg-white/30"
                  style={{
              ...
              {chart.data.length >= 1 ? (
                <svg width={chart.W} height={chart.H} viewBox={`0 0 ${chart.W} ${chart.H}`} className="block">
                  ...
                  <polyline
                    ...
                    fill="none"
                    stroke="currentColor"
                    className="text-black/60 dark:text-white/45"
                    strokeWidth="2.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                ...
                <div className={`py-6 text-center text-sm ${subtle}`}>데이터가 더 쌓이면 그래프가 보여!</div>
              )}
            </div>
