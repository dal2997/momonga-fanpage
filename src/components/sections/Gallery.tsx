...
import type { GalleryItem } from "@/data/characters";
    ...
    <section id="gallery" className="scroll-mt-24">
      <div className="flex items-end justify-between">
        <h2 className={`text-2xl font-semibold ${title}`}>
          {characterName} · 팬들이 좋아하는 순간
        </h2>
        <p className={`text-sm ${subtle}`}>hover 해봐</p>
      </div>

      {items.length === 0 ? (
        <div className={`mt-6 rounded-2xl border border-black/10 bg-black/[0.03] p-12 text-center ${subtle} dark:border-white/10 dark:bg-white/[0.03]`}>
          <p className="text-lg">아직 등록된 순간이 없어</p>
          <p className="mt-2 text-sm">
            <code className="rounded bg-black/5 px-2 py-0.5 dark:bg-white/10">
              src/data/characters.ts
            </code>
            의 <strong>{characterName}</strong> gallery 배열에 이미지를 추가해봐
          </p>
        </div>
      ) : (
